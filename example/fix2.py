...
content = re.sub(r'(\s+)\),\s*=>\s*setState\(\(\)\s*=>\s*_selected\s*=\s*val\),\s*\n\s*\),', r'\1', content)

# Fix the missed _buildRadioOption which has an escaped quote:
# _buildRadioOption(
#   value: 'first_time',
#   label: 'No, I haven\'t applied before',
...
def repl_radio(m):
    ...
    # fix the escaped quote in label if any
    label = label.replace("\\'", "'")
...
# Use a better regex for label to handle escaped quotes, e.g. (.*?) up to ', subtitle:
pattern = re.compile(r'( +)\_buildRadioOption\(\s*value:\s*\'([^\']+)\',\s*label:\s*\'(.*?)\',\s*subtitle:\s*\'(.*?)\',\s*groupValue:.*?,\s*onChanged:.*?,\s*\),', re.DOTALL)
